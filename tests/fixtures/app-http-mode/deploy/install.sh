...
helm upgrade -i "${APP_NAME}" ./charts/${APP_NAME} -n example-system --create-namespace "${values_args[@]}" --set-string "cloudDomain=example.test" --set-string "cloudPort=443" --set-string "httpPort=80" --set-string "disableHttps=true" --set-string "certSecretName=example-cert"
